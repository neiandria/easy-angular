...
import { DataService, Consulta, Paciente, Medico, UsuarioLogado } from '../../services/data-service.service';
...
ngOnInit(): void {
  ...
  // carrega consultas e força filtro de hoje
  this.dataService.getConsultas().subscribe(cs => {
    ...
    this.calcularHoje();
    this.view = 'hoje'; // <- garante que o filtro “Hoje” seja aplicado imediatamente
  });
}
  ...
  get filtradas(): ConsultaExibicao[] {
    ...
    return list.filter(item => {
      const paciente = this.pacientes.find(p => p.id_paciente===item.id_paciente);
    ...
    }).map(c => ({ consulta: c, paciente: this.pacientes.find(p=>p.id_paciente===c.id_paciente)!, medico: this.medicos.find(m=>m.id_medico===c.id_medico)! }));
  }
